...
//DEBUGcout << "inside func:" << __func__ << "tracename: " <<tracename <<endl;
...
//DEBUGcout << &argv[trace_start]<< " "<<argv[trace_start] << endl;
    std::vector<const char*> files(&argv[trace_start], &argv[argc]);
    configs.set_core_num(argc - trace_start);
//DEBUGcout <<"files.size(): "<< files.size()<<". " <<"argc: "<<argc <<endl;
//DEBUGcout << &argv[argc] << "  " <<argv[argc] <<endl; // this line will eliminate all cout below. Why?
//DEBUGcout << "standrad ==" <<standard <<endl;
//std::cout << files[0] <<endl;
